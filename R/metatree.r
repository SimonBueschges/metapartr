...
#' @import Rcpp
#' @export
function (formula, data, vi, subset, c = 1, control = rpart.control(xval = 10,
                                                                    minbucket = 3, minsplit = 6, cp = 1e-04), ...)
{
  Call <- match.call()
  wts.metacart <- NULL
  indx <- match(c("formula", "data", "vi",
                  "subset"), names(Call), nomatch = 0L)
  if (indx[1] == 0L)
  ...
  tree <- rpart(formula, weights = wts.metacart, data = m,
                control = control, ...)
  ...
  if (length(unique(prunedtree$where)) < 2) {
    ...
    res <- list(n = n, Q = Q, df = df, pval.Q = pval.Q, g = g,
